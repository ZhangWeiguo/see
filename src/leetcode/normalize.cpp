...
void Normalize() {
  ...
  for (a = 0; a < num_vertices; a++) {
    ...
    fprintf(fo, "%s ", name);
    if (binary) {
      for (b = 0; b < vector_dim; b++) fwrite(&vec[b], sizeof(real), 1, fo);
    } else {
      for (b = 0; b < vector_dim; b++) fprintf(fo, "%lf ", vec[b]);
    }
    fprintf(fo, "\n");
  }
  free(vec);
...
int main(int argc, char** argv) {
  ...
  // 	printf("\t-binary <int>\n");
  // 	printf("\t\tSave the learnt embeddings in binary moded; default is 0
  // (off)\n"); 	printf("\nExamples:\n"); 	printf("./normalize -input
  // vec_wo_norm.txt -output vec_norm.txt -binary 1\n\n"); 	return 0;
  // }
  // if ((i = ArgPos((char *)"-input", argc, argv)) > 0) strcpy(input_file,
  // argv[i + 1]); if ((i = ArgPos((char *)"-output", argc, argv)) > 0)
  // strcpy(output_file, argv[i + 1]); if ((i = ArgPos((char *)"-binary", argc,
  // argv)) > 0) binary = atoi(argv[i + 1]); Normalize();
  cout << argc << endl;
