...
void TestTreeRegression() {
  ...
  see::TreeRegression tree;
  vector<see::FeatureType> ft = {see::FEATURE_NUM, see::FEATURE_NUM,
                                 see::FEATURE_NUM, see::FEATURE_NUM,
                                 see::FEATURE_NUM, see::FEATURE_NUM};
  tree.SetFeatureType(ft);
  tree.Train(&train_data);
  double train_score = tree.Score(&train_data);
  double test_score = tree.Score(&test_data);
  ...
  test_score = tree.Score(&test_data);
  cout << "Test  Score: " << test_score << " " << test_data.GetSize() << endl;
}

void TestTreeClassifier() {
  ...
  if (feature_s.size() > 0 && target_s.size() > 0) {
    ...
    see::TreeClassifier tree(see::GAIN);
    vector<see::FeatureType> ft = {see::FEATURE_NUM, see::FEATURE_NUM,
                                   see::FEATURE_NUM, see::FEATURE_NUM};
    tree.SetFeatureType(ft);
    tree.Train(&train_data);
